/***********************************************************************************************************
Le présent projet est en licence Crative Commons BY-NC-SA 3.0, Romuald THION. LG 2018
...
//format of csv is //id,type,title,type_carte,picture,credits,decl,init,usage
...
function pict_filename(card_obj, suffix = ''){
  return latex_path + card_obj.id + '_' + clean_french(card_obj.title) + suffix + path.extname(card_obj.picture);
};
...
function front_content(card_obj){
  ...
  \\begin{tikzpicture}
    ...
    ${card_type(card_obj.type)}{${card_obj.card_type}}
    \\cardtitle{${card_obj.title}}
    \\cardcontent{Info}{${card_obj.info}\\\\{\\bf D\\'{e}claration}${card_obj.decl}}
...
function back_content(card_obj){
  ...
  \\begin{tikzpicture}
    ...
    ${card_type(card_obj.type)}{${card_obj.card_type}}
    \\cardtitle{${card_obj.title}}
    \\cardcontent{Fonction init}{${card_obj.init}}
    \\cardcontent{Fonction loop}{${card_obj.usage}}
